          ...
          if (args.length >= 2) {
            ...
            if (
              ...
              const newProps = secondArg.properties.map((props) => {
                ...
                // property name can be identifier or string literal
                const propName = ts.isIdentifier(props.name)
                  ? props.name.text
                  : ts.isStringLiteral(props.name)
                    ? props.name.text
                    : undefined;

                if (propName !== 'name') return props;

                const init = props.initializer;
              ...
              if (changed) {
                const updatedObj = ts.factory.updateObjectLiteralExpression(
                  secondArg,
                  newProps,
                );
                newNode = ts.factory.updateCallExpression(
